...
export async function GET(request: NextRequest) {
  ...
  let redirectUrl = requestUrl.searchParams.get('redirectedFrom') || '/dashboard'
  try {
  ...
  if (code) {
        ...
        process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
        {
          cookies: {
            get(name) {
              ...
              console.log('Auth callback: Reading cookie', { name, value: cookie?.value ? '(exists)' : '(not found)' })
              return cookie?.value
            },
    ...
    console.log('Auth callback: No code parameter found, this may indicate an invalid authentication attempt')
  }
